...
// Сколько разговора отдаём модели. Ограничение нужно только как страховка:
// чужой клиент может прислать сколь угодно длинную «историю», а платим за
// неё мы. Для настоящего звонка эти пределы недостижимы.
const MAX_HISTORY_MESSAGES = 120;
const MAX_HISTORY_CHARS = 24000;

function trimHistory(history) {
  let kept = history.slice(-MAX_HISTORY_MESSAGES);
  let total = kept.reduce((sum, m) => sum + String(m.content ?? '').length, 0);
  // Если реплики оказались непривычно длинными — отрезаем самые старые,
  // пока не уложимся. Свежие важнее: в них текущая тема разговора.
  while (kept.length > 2 && total > MAX_HISTORY_CHARS) {
    total -= String(kept[0].content ?? '').length;
    kept = kept.slice(1);
  }
  return kept;
}
