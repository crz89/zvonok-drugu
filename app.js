...
// Цепочка моделей: лимиты у Groq считаются ОТДЕЛЬНО по каждой, поэтому
// упёршись в одну, просто переходим к следующей. Порядок — по качеству,
// а не по щедрости лимита: ребёнок должен получать лучший ответ, пока это
// возможно. Цифры из панели Groq (Organization Limits), бесплатный тариф:
//
//   llama-3.3-70b-versatile — 100K токенов/сутки. Лучшая связность и самый
//                             жёсткий лимит из всех: примерно 43 реплики.
//   groq/compound           — БЕЗ лимита токенов, 250 запросов/сутки.
//                             Отвечает в характере не хуже, 1.4с.
//   llama-3.1-8b-instant    — 500K токенов/сутки, 14.4K запросов. Слабее
//                             в связности, зато мгновенная (0.3с).
//
// Не берём: groq/compound-mini внутри ходит в ту же 70b и наследует её
// лимит; gpt-oss-120b вернул пустой ответ; qwen3.6 выводит служебные
// рассуждения <think> прямо в реплику ребёнку.
const GROQ_MODELS = [
  GROQ_MODEL,
  'groq/compound',
...
async function generateReplyGroq(character, history) {
  ...
  // Сначала перебираем все ключи на лучшей модели, и только когда ВСЕ
  // упёрлись в её суточный лимит — те же ключи на следующей: лимит
  // считается отдельно по каждой паре «ключ + модель».
